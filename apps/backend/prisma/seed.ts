...
async function main() {
    ...
    await prisma.pfcUser.create({
      data: {
        ...
        individualProfile: {
          create: {
            housingType:
              i % 2 === 0 ? HousingType.apartment : HousingType.house,
            surface: 40 + i * 10,
            haveGarden: i % 2 !== 0,
            haveAnimals: i > 2,
            ...
            availableTime:
              "Disponible le soir et les week-ends pour de longues balades.",
          },
        },
      },
    });
  ...
  for (const shelter of shelters) {
    for (let i = 0; i < 5; i++) {
      ...
      const keyword =
        species.name.toLowerCase() === "chien"
          ? "dog"
          : species.name.toLowerCase() === "chat"
          ? "cat"
          : "rabbit";
      ...
      const treatment =
        animalIndex % 2 === 0 ? medicalNotes[i % medicalNotes.length] : null;
